...
const OPTIMAL_RATIOS = {
  400: [0.254, 0.233, 0.244, 0.269],
  200: [0.520, 0.480],
  100: [0.569, 0.431],
}

export const getTargetSplits = (distance, goalTime) => {
  const ratios = OPTIMAL_RATIOS[distance]
  const targets = TARGETS[distance]
  return ratios.map((ratio, i) => ({
    label: targets[i].label,
    time: Math.round(goalTime * ratio * 100) / 100,
    pct: Math.round(ratio * 1000) / 10,
    greenRange: targets[i].green,
  }))
}
