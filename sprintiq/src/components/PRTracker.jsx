...
export default function PRTracker({ onViewRace }) {
  ...
  const chartData = {}
  ;[100, 200, 400].forEach(d => {
    chartData[d] = byDistance[d].map(r => ({
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-24 pb-12">
      ...
      {[400, 200, 100].filter(d => chartData[d].length >= 2).map(d => (
        <div key={d} className="mb-10">
          <h2 className="text-sm font-semibold text-white/40 uppercase tracking-widest mb-4">{d}m Progression</h2>
          <div className="bg-white/3 rounded-xl p-4">
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={chartData[d]} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
